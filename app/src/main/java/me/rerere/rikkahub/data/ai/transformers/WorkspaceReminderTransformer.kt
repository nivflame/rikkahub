...
private fun buildWorkspacePrompt(workspace: WorkspaceEntity, cwd: String? = null): String = buildString {
    ...
    appendLine("  - `workspace_read`: read file contents.")
    appendLine("  - `workspace_write` / `workspace_edit`: create files, or make precise edits to existing files.")
    appendLine("  - `workspace_bash`: run shell commands (the files area is mounted at /workspace).")
    appendLine("- Prefer `workspace_bash` for tasks that standard Unix tools handle well, and prefer `workspace_edit` for targeted edits over rewriting whole files.")
